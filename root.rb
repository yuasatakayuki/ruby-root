...
Root::gStyle.SetTitleFont(43, "t") # Helvetica
Root::gStyle.SetTitleSize(18, "t") # Helvetica
...
def create_canvas(name="c",title="Canvas",w=DefaultCanvasWidth,h=DefaultCanvasHeight)
	c=Root::TCanvas.create(name.to_s,title,w,h)
	return c
end

def create_file(file_name)
	return Root::TFile.open(file_name,"recreate")
end

def open_file(file_name)
	return Root::TFile.open(file_name)
end
...
class Root::TFile
	def get(name)
		return self.Get(name.to_s)
	end
end

class Root::TDirectoryFile
	def get(name)
		return self.Get(name.to_s)
	end
end
...
module Root::Plottable
	...
	def label(option)
		option.each(){|k,v|
			if(k.to_s.downcase.include?("x"))then
				xlabel(v)
			elsif(k.to_s.downcase.include?("y"))then
				ylabel(v)
			end
		}
	end
	...
	def font_size(size_in_pixel)
		fontSize=size_in_pixel
		self.GetXaxis().SetLabelSize(fontSize)
		self.GetYaxis().SetLabelSize(fontSize)
		self.GetZaxis().SetLabelSize(fontSize)
		self.GetXaxis().SetTitleSize(fontSize)
		self.GetYaxis().SetTitleSize(fontSize)
		self.GetZaxis().SetTitleSize(fontSize)
		@font_size_manually_set=true
	end

	def is_font_size_set()
		if(@font_size_manually_set)then
			return true
		else
			return false
		end
	...
	def font(font_style="helvetica")
		...
		case font_style
		when "helvetica" then fontStyle=43
		when "times" then fontStyle=133
		when "roman" then fontStyle=133
		else
			puts "Error: font #{font_style} invalid"
			exit
		end
		self.GetXaxis().SetLabelFont(fontStyle)
		self.GetYaxis().SetLabelFont(fontStyle)
		self.GetZaxis().SetLabelFont(fontStyle)
		self.GetXaxis().SetTitleFont(fontStyle)
		self.GetYaxis().SetTitleFont(fontStyle)
		self.GetZaxis().SetTitleFont(fontStyle)
		#self.GetXaxis().SetTitleOffset(1)
		#self.GetYaxis().SetTitleOffset(1)
		if(@font_size_manually_set!=true)then
			self.font_size 18
		end
	end
	...
	def marker(marker="circle")
		...
		if(marker.include?("open"))then
			marker.gsub("open","").gsub("_","").strip!
			case marker
			...
			marker.gsub("filled","").gsub("_","").strip!
			case marker
	...
	def color(color)
		if(@@colorMap[color]!=nil)then
			rgb=@@colorMap[color].map(){|e| e/255.0}
	...
	def more_log_labels(option="xy")
		option=option.to_s
		option.downcase!
		if(option.include?("x"))then
			self.GetXaxis().SetMoreLogLabels()
		end
		if(option.include?("y"))then
			self.GetYaxis().SetMoreLogLabels()
		end
	end
	...
	def no_exponent(option="xyz")
		...
		option.downcase!
		if(option.include?("x"))then
			self.GetXaxis().SetNoExponent()
		end
		if(option.include?("y"))then
			self.GetYaxis().SetNoExponent()
		end
		if(option.include?("z"))then
			self.GetZaxis().SetNoExponent()
		end
	end
	...
	def range(x,y=[])
		if(x.instance_of?(Array) and x.length==2)then
			self.GetXaxis().SetRangeUser(x[0],x[1])
			if(y.instance_of?(Array) and y.length==2)then
				self.GetYaxis().SetRangeUser(y[0],y[1])
			end
		else
			if(x.instance_of?(Fixnum) and !y.instance_of?(Array))then
				x_lower=x.to_f
				x_upper=y.to_f
...
module PlotManipulator
	private
	def set_font_if_nil()
		if(@fontStyle==0 or @fontStyle==nil)then
			@fontStyle=43
			@fontSize=12
		end
	end
...
module PadManipulator
	...
	def log(axes="xyz")
		axes.downcase!
		if axes.include?("x") then
			self.logx()
		end
		if axes.include?("y") then
			self.logy()
		end
		if axes.include?("z") then
			self.logz()
		end
	end
	def logx(mode=1)
	...
	def divide_top_bottom(top_fraction=0.7)
		if(top_fraction>=1)then
			STDERR.puts "Error: Root::TCanvas.divide_top_bottom() only accepts 0<top_fraction<1 ."
			exit -1
		end
		@@Minification=0.95
		self.cd(0)
		self.Divide(1,2)
		self.GetPad(1).SetPad(0,(1-top_fraction)*@@Minification,1*@@Minification,1*@@Minification)
		self.GetPad(2).SetPad(0,0,1*@@Minification,(1-top_fraction)*@@Minification)
	...
	def get_margin()
		return [get_margin_top, get_margin_right, get_margin_bottom, get_margin_left]		
	end
...
module HistogramManipulationInterface
	def clone(new_name)
		xaxis=self.GetXaxis()
		h=TH1D.create(new_name,new_name,xaxis.GetNbins(), xaxis.GetXmin(), xaxis.GetXmax())
	...
	def dump_hist_bin_error(divider)
		str=<<EOS
   self.nbins = #{self.nbins} xmin = #{self.xmin} xmax = #{self.xmax}, divider.nbins = #{divider.nbins} xmin = #{divider.xmin} xmax = #{divider.xmax}
EOS
		return str
	end

	public
	def divide_by(hist, new_name="")
		# if(new_name=="")then
		# 	new_name=self.GetName()+"_divided_by_"+hist.GetName()
		# end
		# xaxis=self.GetXaxis()
		# h=TH1D.create(new_name,new_name,xaxis.GetNbins(), xaxis.GetXmin(), xaxis.GetXmax())
		if(self.GetXaxis().GetNbins!=hist.GetXaxis().GetNbins)then
			raise "Error: divide_by(): two histograms have different X axis bin number, and bin-by-bin division cannot be executed. #{self.dump_hist_bin_error(hist)}"
		end
		for i in 0..(self.GetXaxis().GetNbins())
			if(self.GetXaxis().GetBinCenter(i)!=hist.GetXaxis().GetBinCenter(i))then
				raise "Error: divide_by(): two histograms have different X axis binning (at bin #{i}, binters are #{self.GetXaxis().GetBinCenter(i)}!=#{hist.GetXaxis().GetBinCenter(i)}, and bin-by-bin division cannot be executed. #{self.dump_hist_bin_error(hist)}"
			end
			a=self.GetBinContent(i)
			b=hist.GetBinContent(i)
			if(b!=0)then
				self.SetBinContent(i,a/b)
			end
		end
		#return h
	end
	...
	def xmin
		return self.GetXaxis().GetXmin()
	end
	def xmax
		return self.GetXaxis().GetXmax()
	end
end
...
def load_data_as_graph(file_name, xycolumns=[0,1], delimitter=" ")
	puts "Creating graph object from #{file_name} using columns #{xycolumns.join(" and ")}"
	graph_name="graph_"+File.basename(file_name,".*")
	g=Root::TGraph.create([],[])
	g.SetName(graph_name)
	
	xcolumnIndex=xycolumns[0]
	ycolumnIndex=xycolumns[1]

	open(file_name).each(){|line|
		array=line.split(delimitter)
		x=array[xcolumnIndex].to_f
		y=array[ycolumnIndex].to_f
